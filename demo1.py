...
with col1:
    st.write(f"{app_mode} Pose by EL MEMBORSSS")
...
with col2:
    ...
    if start:
        ...
        stream_url = "http://192.168.124.182/capture"

        # Function to fetch frames from the stream
        def get_frame_from_stream(url):
            try:
                ...
                st.warning(f"Failed to retrieve image: {e}")
                return None

        # Processing logic
        with mp_pose.Pose(min_detection_confidence=0.5, min_tracking_confidence=0.5) as pose:
            while True:
                img = get_frame_from_stream(stream_url)
                if img is not None:
                    h, w, c = img.shape
